  ...
  try {
    for (const team of teams) {
      ...
      await prisma.esportsTeams.upsert({
        ...
        update: { teamTag: team.tag, logo: `${process.env.CDN_BASE_URL ?? 'https://clarioncorp.net'}/teams/${series}/${team.tag}.webp` }, // Always use latest icon for now
        create: {
          teamTag: team.tag,
          teamName: team.name,
          series,
          season,
          logo: `${process.env.CDN_BASE_URL ?? 'https://clarioncorp.net'}/teams/${series}/${team.tag}.webp`
        }
      });
